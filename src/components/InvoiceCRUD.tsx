...
const InvoiceCRUD: FC = () => {
  ...
  const {
    ...
    updateItem,
    deleteItem
  } = useApiData<Invoice>({ endpoint: 'invoices' });
  ...
  const handleUpdate = async (values: any) => {
    ...
    try {
      const totalAmount = values.amount * (1 + values.tax_rate / 100);
      await updateItem(editingInvoice.id, { ...values, total_amount: totalAmount });
      setShowModal(false);
  ...
  const handleEdit = (invoice: Invoice) => {
    ...
    Object.keys(formData).forEach(key => {
      setFieldValue(key, invoice[key as keyof Invoice] || '');
    });
    setShowModal(true);
  };
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filteredInvoices.map((invoice) => (
                <tr key={invoice.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm text-gray-900">{getClientName(invoice.client_id)}</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    <div className="text-sm text-gray-500">
                      HT: {invoice.amount.toLocaleString('fr-FR')}€
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
        ...
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
          <div className="relative top-10 mx-auto p-5 border w-full max-w-2xl shadow-lg rounded-md bg-white">
            <div className="mt-3">
              ...
              <form onSubmit={(e) => {
                e.preventDefault();
                handleSubmit(editingInvoice ? handleUpdate : handleCreate);
              }} className="space-y-4">
                ...
                <div className="flex justify-end space-x-3 pt-4">
                  ...
                  >
                    {isSubmitting ? 'Enregistrement...' : (editingInvoice ? 'Modifier' : 'Créer')}
                  </button>
                </div>
              </form>
            </div>
